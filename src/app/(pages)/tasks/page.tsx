...
export default function Tasks() {
  ...
  function deleteTask(task: Task, index: number ) {
  ...
  return <div className="w-full h-full lg:w-[80%] p-4 pt-14">
    ...
    <div className="flex gap-2 pb-4 border-b-2 border-b-cyan-800 border-opacity-10">
      <input
        value={ newTask.name }
        onChange={ (targetElement) => { setNewTask({ name: targetElement.target.value, timeLeft: newTask.timeLeft, stoped: newTask.stoped }) } }
        placeholder="Task name" className="w-[80%]"
      />
      <input
        value={ newTask.timeLeft > 0 ? newTask.timeLeft : ""}
        type="number"
        placeholder="minutes"
        onChange={(targetElement) => { setNewTask({ name: newTask.name, timeLeft: Number.parseInt(targetElement.target.value), stoped: newTask.stoped }) } }
        className="w-[10%]"
      />
      <button className="w-[10%] bg-cyan-700 p-1 text-cyan-100 font-bold hover:bg-yellow-500" onClick={() => { setTasks([...tasks, newTask]) }}>ADD</button>
    </div>
    ...
    <div className="mt-4 bg-cyan-700 p-[1px]">
      ...
      <ul>
        {tasks.map((task,index) => (
          <TaskItemList index={index} name={task.name} stoped={task.stoped} timeLeft={task.timeLeft} delete={deleteTask} finishedTask={() => { audioAlarmRef?.current?.play(); }} />
        ))}
      </ul>
    </div>
